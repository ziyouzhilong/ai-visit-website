...
4. Call `page_read` for public candidates. If it returns a hard automated-client boundary and the user has paired and authorized Chrome, call `browser_status` before any `browser_page_read` or `browser_batch_start`. Proceed only when both `configured: true` and `connected: true`. If the first status is not connected and its failure is retryable, wait 5-10 seconds and call `browser_status` once more. If the second check is still not connected, stop the browser-reading path, report the exact status/failure, and ask the user to verify Chrome, extension enablement, pairing, and the authorized origin. Do not require the extension Settings page to remain open. Use `browser_page_read` for one selected URL or `browser_batch_start` for up to 20 selected URLs. Never switch to Chrome merely to evade a CAPTCHA or bypass access controls.
...
Call `browser_status` before the first authenticated browser read. Continue only when the result has both `configured: true` and `connected: true`. If the first check is offline with a retryable failure, wait 5-10 seconds and retry `browser_status` once; do not start a page read or batch while disconnected. If the retry also fails, stop and report the exact fields and failure instead of repeatedly retrying or silently falling back. The extension Settings page is needed only to configure or diagnose pairing and does not need to stay open. A connected status proves only that the paired extension is online; it does not prove the requested website is logged in or authorized.
